...
} else if (params.assay == ["exome", "exome-no-manta-no-phase"]) {
...
process run_pipeline {
  ...
  publishDir "${output_base}/${sample}/${assay_type}/analyses/${uuid}/", pattern: "inputs/params.json", mode: 'copy', overwrite: 'true', saveAs: { f -> "params.json"}
